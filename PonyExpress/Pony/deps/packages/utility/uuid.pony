...
class val UUID is Equatable[UUID]
  ...
  fun tag _create_v4(): Array[U8] val =>
    let size = Size()
    let bytes = recover Array[U8](size) end
	let r1 = @arc4random[U32]().u128()
	let r2 = @arc4random[U32]().u128()
	let r3 = @arc4random[U32]().u128()
	let r4 = @arc4random[U32]().u128()
	
	let r = (r1 << 96) or (r2 << 64) or (r3 << 32) or (r4)
...
type Namespace is (_DNS | _URL | _OID | X500)
  """
